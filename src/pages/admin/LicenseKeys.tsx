...
export function AdminLicenseKeys() {
  ...
  async function load() {
    ...
    const { data, error: e } = await supabase
      .from("license_keys")
      .select("id, code, grant_type, duration_hours, redeemed_by_user_id, redeemed_at, created_at")
  ...
  const activeKeys = keys.filter((k) => !k.redeemed_at);
  const redeemedKeys = keys.filter((k) => k.redeemed_at);
  ...
  return (
    <div>
      ...
      <p className="admin-page-subtitle">
        {loading
          ? "Loading..."
          : `${activeKeys.length} unredeemed · ${redeemedKeys.length} redeemed`}
      </p>

      <div className="admin-section">
        <div className="admin-section__title">Generate keys</div>
        <div className="admin-section__hint">
          Codes look like <code>PREFIX-XXXX-XXXX-XXXX-XXXX</code>. Single-use.
        </div>
        <form onSubmit={handleCreate}>
          ...
          <button type="submit" className="btn btn-primary" disabled={creating}>
            {creating ? "Creating..." : `Create ${quantity} key${quantity === 1 ? "" : "s"}`}
          </button>
        </form>
            ...
            {keys.map((k) => (
              <tr key={k.id}>
                ...
                <td>
                  {k.redeemed_at ? (
                    <>
                      <span className="admin-badge admin-badge--dim">Redeemed</span>
